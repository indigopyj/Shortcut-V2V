...
parser.add_argument('--target_path', action="store", type=str, default="/home/nas4_user/yeojeongpark/compression/Viper/data/cycle-gan/val/B/")
...
parser.add_argument("--omgd", action='store_true')
args = parser.parse_args()

device = "cuda" if torch.cuda.is_available() else "cpu"

fns = os.path.join(args.exp_name, "*", "*.png")
fs = glob.glob(fns)
...
for f in (fs):
    ...
    if args.omgd:
        img_ind = int(os.path.basename(f).split("_")[1])    
    else:
        img_ind = int(os.path.basename(f).split("_")[1][:-4])
    
    vid_name = "%s_%05d.png" %(vid_ind, img_ind)
    target_path = os.path.join(args.target_path, vid_name)
    # print(target_path)
    gt_cache_dir = os.path.join(args.lbl_cache_dir, vid_name)
