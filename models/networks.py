...
def weights_init_zero(m):
    ...
    #print(classname)
    if classname.find('Deformable') != -1:
        pass 
...
def weights_init_xavier(m):
    ...
    if classname.find('Deformable') != -1:
        pass
    elif classname.find('HetConv') != -1:
        init.xavier_normal(m.conv3x3.weight.data, gain=0.02)
        # init.xavier_normal(m.conv3x3.weight.data, gain=0.02)
        init.xavier_normal(m.conv1x1_.weight.data, gain=0.02)
        # init.xavier_normal(m.conv1x1_.weight.data, gain=0.02)
        init.xavier_normal(m.conv1x1.weight.data, gain=0.02)
        # init.xavier_normal(m.conv1x1.weight.data, gain=0.02)
    elif hasattr(m, 'weight') and classname.find('Conv') != -1:
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    elif which_model_netG == 'mobile_resnet_9blocks':
    ...
    elif which_model_netG == 'mobile_resnet_6blocks':
        netG = MobileResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6, gpu_ids=gpu_ids)
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    elif which_model_netD == 'temporal_patchGAN':
        netD = TemporalPatchGANDiscriminator(input_nc, ndf, n_layers=n_layers_D, use_sigmoid=use_sigmoid, gpu_ids=gpu_ids)
    else:
...
class MobileResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, gpu_ids=[], padding_type='reflect'):
        ...
        mult = 2**n_downsampling
        for i in range(n_blocks):# Define a resnet block
...
class TemporalPatchGANDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=None, use_sigmoid=False, gpu_ids=[]):
        ...
        # default norm : Spectral normalization
        self.gpu_ids = gpu_ids

        kw = (3, 4, 4)
        padw = (1, 1, 1)
        sequence = [
            nn.utils.spectral_norm(nn.Conv3d(input_nc, ndf, kernel_size=kw, stride=(1, 2, 2), padding=padw)),
            nn.LeakyReLU(0.2, True)
        ]
        ...
        sequence += [nn.Conv3d(ndf * nf_mult, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw)]
